...
이 파일은 `db-pipeline`의 물리적인 개발 환경, 폴더 구조, 실행 명령어 규약 및 데이터 정제/적재 파이프라인을 정의합니다. 에이전트와 개발자는 작업을 시작하기 전 이 사양을 반드시 숙지해야 합니다.
...
본 파이프라인은 LH OpenAPI 등을 통해 수집한 분양/임대 공고문(PDF, Excel)을 마크다운으로 구조화한 후, 본체 에이전트가 직접 data.json을 추출하고, 원본 document.md와의 1:1 교차 대조 검증을 거쳐 SQLite3 관계형 DB([public_housing.db](file:///home/iru/app/pleasehome/db-pipeline/public_housing.db))에 최종 적재하는 파이프라인입니다.
...
graph TD
    ...
    B --> C[3. 본체 에이전트 data.json 직접 추출]
    C --> D{4. 1:1 시맨틱 교차 대조 검증}
...
* **수행 모듈**: [api.py](file:///home/iru/app/pleasehome/db-pipeline/src/lh_notice/api.py), [main.py](file:///home/iru/app/pleasehome/db-pipeline/src/lh_notice/main.py)
* **동작 방식**:
  ...
  * 동시에 수집 대상의 메타데이터(공고명, 청약유형, 상세URL 등)를 [download_meta.json](file:///home/iru/app/pleasehome/db-pipeline/docs/pdf/download_meta.json)에 기록하여 후속 단계에서 연계할 수 있게 보존합니다.

#### 2단계: PDF/엑셀의 표준 마크다운 변환 및 정리
* **수행 모듈**: [convert_pdf_to_md.py](file:///home/iru/app/pleasehome/db-pipeline/.agents/scripts/convert_pdf_to_md.py)
* **동작 방식**:
  * `docs/pdf/`에 다운로드된 원천 문서들을 분석하여 고유 표준 공고 폴더명(`{기관명}_{공고ID}_{공고일자}`)을 생성하고, 파일들을 해당 폴더로 자동 그룹화 이동합니다.
...
#### 3단계: 본체 에이전트의 정제 JSON 데이터 직접 추출
* **수행 모듈**: 본체 에이전트 (Antigravity)
* **동작 방식**:
  * [document.md](file:///home/iru/app/pleasehome/db-pipeline/docs/md/{공고_폴더}/document.md)를 바탕으로 비즈니스 룰 및 DB 제약 조건에 적합한 JSON 스펙 데이터를 추출하여 [data.json](file:///home/iru/app/pleasehome/db-pipeline/docs/md/{공고_폴더}/data.json)으로 저장합니다.
  * 예비 입주자 전용 공고 등 공고의 목적에 맞추어 유닛의 공급 수(`supply_count`) 및 예비 수(`reserve_count`)를 1:1 매핑하며, 비정형 주소 사용 여부가 발견되면 웹 검색을 가동해 정식 번지 주소를 보완합니다.

#### 4단계: 1:1 시맨틱 교차 대조 검증
* **수행 모듈**: 본체 에이전트 (Antigravity)
* **동작 방식**:
  * DB 적재 직전에 에이전트가 직접 document.md 원본 내의 모든 임대 조건 표와 생성된 data.json을 1:1 교차 대조하여 누락 및 수치 불일치 정합성을 정밀 검사합니다.
  * 17대 광역지자체 공식 지역명 및 시행기관 매핑 상태 검증.
  * 날짜 형식 준수 및 순서 논리(신청접수 → 당첨자발표 → 계약체결) 검증.
  * 상호전환 공식(연 6.0% 및 연 6.7% 증액 범위와 연 2.5% 및 연 3.5% 감액 범위)과 보증금-임대료 변동 수학적 정합성을 정밀 대조합니다.
  * 오류 발생 시 본체 에이전트가 data.json을 재작업 및 보완합니다.

#### 5단계: DB 적재 및 안전 격리 (Fail-Safe)
* **수행 모듈**: [insert_loader.py](file:///home/iru/app/pleasehome/db-pipeline/.agents/scripts/insert_loader.py)
* **동작 방식**:
  ...
  * **우아한 성능 저하 적재 (FAIL - Fail-Safe)**: 파싱에 최종 실패하거나 정합성 에러가 해결되지 않을 경우, `insert_loader.py`에 `--status FAIL` 옵션을 주어 실행합니다. 이 모드에서는 announcements 테이블에 최소 메타데이터만 기재하고, 관계형 데이터 대신 세부 조회용 `announcement_details` 테이블에 공고문 원본 마크다운을 통째로 적재해 두어 사용자가 원본 조회를 할 수 있도록 설계되어 있습니다.
...
### 4.3. 추출 데이터 1:1 대조 검증 및 DB 적재
```bash
# 1. 1:1 시맨틱 교차 대조 검증 (에이전트가 직접 document.md와 data.json 내용 비교)

# 2. 검증 완료 시 DB 정상 적재 완결 (source와 backup 경로 지정)
