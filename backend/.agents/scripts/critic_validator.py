...
def validate_announcement(data, features):
    ...
    if not region:
        errors.append("공고의 대표 지역명(region)이 누락되었습니다.")
    elif region not in valid_regions:
        errors.append(f"유효하지 않은 지역명입니다: '{region}'. ({', '.join(valid_regions)} 중 하나여야 합니다.)")
