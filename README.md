...
### 2. 지도 기반 대시보드 (web)
* **공고 목록 API N+1 쿼리 최적화**: 공고 세부 정보/일정/제한사항 개별 쿼리 조회를 In-Memory 대량 조회 방식으로 전환하여 SQL 쿼리 수를 300+개에서 4개로 대폭 절감 및 응답 속도를 최적화했습니다.
* **실시간 지리 시각화 (Naver Maps API)**: 네이버 지도 API와 정밀 연동하여, SQLite DB의 위경도 좌표를 기반으로 수백 개의 공공주택 단지 마커를 마커 재사용 최적화 패턴으로 브라우저 레이턴시 없이 즉각 매핑 및 렌더링합니다.
* **서브 탭 모듈화 사이드바**: 사이드바 컴포넌트를 `SearchTab`, `BookmarkTab`, `MoreTab` 단위로 구조적으로 모듈화 분리하여 유지보수성 및 UI 렌더링 성능을 극대화했습니다.
...
* **비로그인 사용자 북마크 로그인 유도**: 로그인하지 않은 상태로 단지 북마크 저장을 시도할 때 저장 팝업 대신 로그인 유도 모달(`AuthModal`)을 호출하여 기존 찜/숨김 버튼과 동일하게 동작하도록 예외 처리를 반영했습니다.
* **기본 북마크 폴더('default') 제거 및 UI/UX 방어**: 기본 폴더('default')의 자동 생성 및 관련 하드코딩 예외 처리를 프로젝트 전반에서 삭제했으며, 폴더 삭제 시 소속 북마크 아이템이 함께 삭제되도록 수정했습니다. 또한, 저장 모달 내에 생성된 폴더가 없을 때를 대비한 안내(Remind) 문구 및 저장 차단(Validation) 로직을 추가했습니다.
...
* **신규 단지 탭(COMPLEX 탭) 및 광역시도 지역 필터**: 모집 공고에 예속된 단지들을 독립된 리스트로 렌더링하고, 8대 광역시급을 개별 분리한 14대 광역지자체 지역 필터 칩을 지원합니다. (단지 카드는 최초 주소까지만 노출되는 고도화된 컴팩트 디자인 채택)
* **지역 필터 지도 마커 연동 및 자동 뷰포트 이동 (PanTo)**: 단지 탭의 지역 필터를 클릭할 때 지도 마커가 해당 지역의 단지들로 즉각 갱신되며, 지도 시점 역시 해당 광역단체의 정규 중심지 좌표(도청/시청 위경도)로 부드럽게 슥 스크롤 이동(PanTo)하도록 결합 제어했습니다.
* **상세 패널 탭별 조건부 분기 및 연관 공고 아코디언 토글**: 우측 상세 패널(`DetailPanel`)을 공용화하되 진입 탭에 따라 하단을 조건부 렌더링(공고 탭: 주택형표, 단지 탭: 연관 공고 카드)합니다. 단지 탭에서 연관 공고 요약 카드를 누르면 탭 전환 없이 제자리에서 청약 일정 및 보증금 한도 등의 아코디언이 접히고 열리는 정밀 접이 토글 상태 관리를 내장했습니다.
* **사이드바 탭 전환 시 상세 패널 자동 닫기 및 URL 클린 롤백**: 탭 전환 시 이전 탭의 상세 조회 상태를 초기화하고 상세 패널을 닫아주며, 브라우저 주소창의 쿼리 스트링(`?announcement_id`, `?complex_id`)도 루트 주소(`/`)로 깨끗이 초기화(롤백)하여 탭 전환 간의 상태 꼬임을 원천 제거했습니다.
* **새로고침 시 쿼리 자동 복구 제거 및 주소창 클린 초기화**: 새로고침 및 최초 진입 시 이전 상세 상태가 꼬이거나 패널이 순간 열리는 비동기 레이스 컨디션을 방지하고자, 쿼리 기반 공고/단지 자동 복구 효과를 걷어내고 최초 마운트 즉시 주소창 쿼리를 깨끗하게 정화하도록 제어했습니다.
* **모바일 및 저장 탭 지도 이동 및 사이드바 동기화 복원**: 모바일 뷰 상세 패널 호출 시 사이드바 접힘에 따른 activeTab null 현상에 대비하여 백업 상태(lastActiveTab)를 지도 마커(mapComplexes) 및 저장/단지 해제 시 리셋 분기, 그리고 폴더 비교 활성화 콜백에 폴백으로 매핑하여 지도 이동 및 바텀시트 가려짐 오작동을 해결했습니다.
* **해(☀️)/달(🌙) 토글 스위치 기반 전역 다크모드**: 더보기 탭에서 해/달 토글 스위치를 통해 네이버 지도 타일 다크 반전 및 슬레이트 톤의 야간 모드 테마를 즉시 전환할 수 있습니다. (설정값은 `theme` 키로 localStorage에 영속화)
