...
1. **폴더 구조 및 이름:** `doc/pdf/` 및 `doc/md/` 하위의 폴더명은 공고 폴더 생성 시 띄어쓰기를 배제하고 언더바(`_`)로 연결하며, **`YYYY_[차수/구분]_[청약유형]`** 포맷(예: `2026_1차_행복주택`)으로 명명합니다.
2. **원본 PDF 파일명:** 폴더 하위에 위치하는 원본 공고문 PDF 파일은 항상 **`origin.pdf`**로 명명합니다.
...
| **`공공임대`** | 공공임대주택 | LH, SH | 일정 기간 임대 거주 후 분양전환하는 임대 |
