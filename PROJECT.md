...
5. **중앙 설정 상수 파일 및 디자인 토큰 이원화 관리:** 리사이저 너비 제한 범위, 상세 필터 초기값, 슬라이더 조작 step 크기, 지도의 좌표 및 줌 제한과 같은 시스템 매직 넘버와 **아이콘 크기, 선 굵기, 여백 규격** 등 인라인 디자인 토큰은 반드시 `src/constants/index.ts`에 상수 객체로 정의하여 중앙 관리합니다. 반면, **폰트 크기와 테마 색상(Hex Code) 팔레트**는 타입스크립트 직접 임포트 제약을 고려하여 `src/app/globals.css`의 전역 CSS 변수로 완전 분리하여 관리합니다. 기존 컴포넌트별 `.module.css` 파일은 수치/색상 하드코딩을 배제하고 오직 레이아웃(Layout) 구조와 상태(Behavior) 제어 목적으로만 기능하도록 구성하며, 상수 값을 덮어씌우는 강제 고정 족쇄 속성(width 등)이 남지 않도록 무결성을 엄격히 보장해야 합니다.
