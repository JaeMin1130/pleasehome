...
5. **중앙 설정 상수 파일 관리:** 리사이저 너비 제한 범위, 상세 필터 초기값, 슬라이더 조작 step 크기, 지도 기본 설정 좌표 및 줌 제한과 같은 프로젝트 내 핵심 매직 넘버들은 각 소스코드에 하드코딩하지 않고 반드시 `src/constants/index.ts`에 정의하여 중앙 관리하여 유지보수성을 극대화합니다.
