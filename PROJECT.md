...
1. **폴더 구조 및 이름:** `docs/pdf/` 및 `docs/md/` 하위의 폴더명은 공고 폴더 생성 시 띄어쓰기를 배제하고 언더바(`_`)로 연결하며, **`YYYY_[차수/구분]_[청약유형]_[기관명]_[시퀀스(01, 02, ...)]`** 포맷(예: `2026_1차_행복주택_LH_01`)으로 명명합니다.
...
4. **정제 데이터 물리 보관 및 이식성 보장**: 서브에이전트가 추출한 JSON 형태의 정제 데이터는 임시 경로에 두지 않고, 각 공고 마크다운 폴더 하위인 `docs/md/{공고_폴더}/data.json`에 물리적인 파일로 보존하여 관리합니다. 또한, 데이터베이스 적재 시 파일 시스템의 절대 경로가 하드코딩되지 않도록 프로젝트 루트 기준 가변 상대 경로로 변환하여 저장함으로써 환경 이식성을 보장합니다.
...
6. **지도시점 이동 및 줌 레벨 조정 제어**: 공고 선택 시 단지 목록이 화면에 나타날 때 지도의 시점 이동과 줌 레벨은 다음 규칙에 따라 제어합니다.
   - 단지가 1개만 존재하는 경우: 줌 레벨 변동 없이 선택된 단지를 지도의 중앙으로 이동합니다.
   - 단지가 여러 개인 경우: 모든 단지가 이미 현재 지도의 가시 영역 내에 존재하면 줌 레벨 변동 없이 중심 좌표만 이동시킵니다.
   - 가시 영역을 벗어난 단지가 존재하는 경우: 지도의 줌 레벨을 `11`로 변경하고 모든 단지의 지리적 중심좌표로 이동시킵니다. (수원 당수 등 인접 다중 단지 시 과도하게 화면이 확대되는 것을 방지하기 위함)
