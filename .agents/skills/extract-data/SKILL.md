...
9. **총 공급규모(공가+예비자) 전수 교차 검증 의무 (Total Household Cross-Check)**:
   * 공고문 서두의 총 모집규모(공가 입주자 + 예비 입주자)와 추출된 유닛별 세대수 합계(`sum(supply_count + reserve_count)`)가 1:1로 일치하는지 반드시 수치 대조 검증을 수행하며, 불일치 시 적재를 즉시 중단하고 원인을 역추적해야 합니다.
...
12. **지구 단위(복수 단지) 공급 시 실제 단지 1:1 분배 매핑**:
    * 공고문이 지구 단위(예: 강일지구 1~10단지, 마곡지구 1~15단지)로 일괄 고지되더라도, `complexes` 테이블의 실제 도로명 주소를 가진 개별 단지마다 해당 공급 유닛(`units`)을 빠짐없이 1:1로 연결하여 단지 상세 화면이 비어 보이지 않도록 매핑합니다.

---

## 4. 상세 요건(details) 6대 표준 카테고리 및 상세화 필수 수록 규격

`details`는 사용자가 공고문 원본(PDF)을 다시 열어보지 않고도 웹 대시보드 화면만으로 청약의 모든 자격·소득·배점·서류를 완벽히 파악할 수 있도록 **풍부하고 완성도 높은 프리미엄 마크다운 문서**로 작성해야 합니다. **단순 3~4줄의 요약이나 하드코딩 템플릿 사용을 엄격히 금지**하며, 공고문 본문(`document.md`)의 세부 수치와 표를 전수 발췌하여 수록해야 합니다.
