...
* **슬라이싱 감지 기준**:
  - `일정`, `신청접수`, `보증금`, `임대료`, `한도액`, `지원기준`, `공급대상`, `동호수`, `면적`, `신청자격` 키워드가 들어간 테이블(`|---|`) 및 헤더 영역을 추출합니다.
  - 정보 누락 방지를 위해 감지된 테이블/본문 시작 전 10라인부터 끝난 후 20라인(비고 각주 포함) 컨텍스트를 병합(Merge)합니다.
  - 공고문 후반부에 수록되는 개인정보 동의서, 위임장 양식 등 불필요한 별첨 텍스트는 슬라이싱 대상에서 원천 절단하여 제외합니다.

### 2단계: 서브에이전트 가동 및 data.json 파일 물리 저장 (서브에이전트)
* **동작**: 부모 에이전트가 슬라이싱된 텍스트와 저장할 로컬 경로(`doc/md/{공고_폴더}/data.json`)를 인자로 전달하여 서브에이전트(`markdown_sql_parser_v2`)를 구동합니다.
