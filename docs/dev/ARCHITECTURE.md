...
* **웹 서빙 및 회원 관리(web)**: Next.js 서버는 공고 DB(`public_housing.db`)를 Read-Only 조회를 수행하고, 회원 계정/찜/숨김/북마크 등 실시간 변경 데이터는 별도의 회원 전용 DB(`user_data.db`)에 격리 보관하여 배포 시 회원 데이터 유실을 완벽히 차단합니다.
